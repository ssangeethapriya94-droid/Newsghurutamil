...
const Home = () => {
  ...
  const fetchAll = async () => {
    try {
      ...
      const [newsRes, configRes, shortsRes, photosRes, videosRes, sponsoredRes] = await Promise.all([
        ...
        API.get("/api/sponsored/published?language=en").catch(() => ({ data: { articles: [] } }))
      ]);
  ...
  const DEFAULT_SECTIONS = [
    ...
    { id: "shorts", titleTa: "Shorts", titleEn: "Shorts", isEnabled: true, order: 10 },
  ...
  const renderSectionVideo = (categorySlug) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ width: "100%" }}>
          {/* YouTubeFacade — loads iframe only on click */}
  ...
  const renderShortsSection = (titleEn) => {
    ...
    return (
      <div style={{ marginBottom: "40px" }}>
        <div className="section-headline-bar">
          <h2 className="section-title-premium">
            <FaMobileAlt style={{ color: "var(--accent-orange)", marginRight: "8px" }} /> {(titleEn || "Shorts").replace(/\s*Reels/gi, "")}
          </h2>
        </div>
  ...
  const renderSponsoredSection = (titleEn) => {
    const list = (sponsoredNews || []).filter(a => a.placement === "homepage_sponsored");
    if (!list || list.length === 0) return null;

    const isTamil = titleEn === "பங்குதாரர் செய்திகள்";
      ...
      <div style={{
        ...
        <div className="section-headline-bar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", borderBottom: "2px solid var(--border-color)", paddingBottom: "10px" }}>
          <h2 className="section-title-premium" style={{ fontSize: "1.3rem", fontWeight: "800", color: "var(--accent-orange)", margin: 0 }}>
            {isTamil ? "ஸ்பான்சர் செய்திகள்" : "Sponsored Content"}
          </h2>
            ...
            return (
              ...
              >
                <div style={{ flex: isLargeScreen ? "1.2" : "1", position: "relative", height: isLargeScreen ? "280px" : "200px", overflow: "hidden" }}>
                  {art.image ? (
                    <img src={art.image} alt={art.title} className="sponsored-zoom-img" style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.5s ease" }} />
                  ) : (
                ...
                <div style={{ flex: "1.8", padding: "24px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border-color)", paddingTop: "14px", marginTop: "auto" }}>
                    ...
                    <span style={{ fontSize: "0.85rem", color: "var(--accent-orange)", fontWeight: "800", display: "flex", alignItems: "center", gap: "4px" }}>
                      {isTamil ? "மேலும் வாசிக்க" : "Read Full Spotlight"} &rarr;
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div style={{ padding: "16px", display: "flex", flexDirection: "column", flex: 1 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto", borderTop: "1px solid var(--border-color)", paddingTop: "12px" }}>
                    ...
                    <span style={{ fontSize: "0.78rem", color: "var(--accent-orange)", fontWeight: "800", display: "flex", alignItems: "center", gap: "2px" }}>
                      {isTamil ? "மேலும் வாசிக்க" : "Read Story"} &rarr;
                    </span>
                  </div>
                </div>
              </div>
  ...
  const renderSectionById = (id, titleEn) => {
    switch (id) {
      ...
      case "sponsored":
        return renderSponsoredSection(titleEn);
      default:
        return null;
    }
  };
      ...
      <div style={{ marginBottom: "40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px solid var(--accent-orange)", paddingBottom: "12px", marginBottom: "25px" }}>
          <div>
            ...
            <p style={{ margin: "5px 0 0 0", fontSize: "0.88rem", color: "var(--text-secondary)" }}>
              Found {matchedNews.length} {matchedNews.length === 1 ? "article" : "articles"} matching "{searchQuery}"
            </p>
          </div>
          <button 
            onClick={() => navigate("/")}
        ...
        {matchedNews.length > 0 ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 20px", background: "var(--bg-secondary)", borderRadius: "12px", border: "1px solid var(--border-color)", marginTop: "20px" }}>
            ...
            <h3 style={{ fontFamily: "var(--font-serif)", fontSize: "1.2rem", color: "var(--text-primary)", margin: "0 0 10px 0" }}>No Results Found</h3>
            <p style={{ margin: 0, fontSize: "0.9rem", color: "var(--text-muted)", maxWidth: "400px", marginLeft: "auto", marginRight: "auto", lineHeight: "1.5" }}>
              We couldn't find any articles matching your search query. Please try searching with different keywords.
            </p>
          </div>
        )}
      </div>
          ...
          >
            ...
            <div style={{ width: isLargeScreen ? "40%" : "100%", height: "220px", borderRadius: "12px", overflow: "hidden", position: "relative", flexShrink: 0 }}>
              ...
              <span style={{ position: "absolute", top: "12px", left: "12px", background: "rgba(234,88,12,0.95)", color: "#fff", padding: "4px 10px", borderRadius: "6px", fontSize: "0.75rem", fontWeight: "800", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                {art.sponsoredLabel || "Featured Partner"}
              </span>
            </div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <div style={{ marginTop: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "0.75rem", opacity: 0.6, fontWeight: "600" }}>Published: {new Date(art.publishedAt || art.createdAt).toLocaleDateString()}</span>
                <button
                  style={{
        ...
        <div ref={leftRef} style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
          {searchQuery ? (
            ...
            <>
              ...
              {renderSponsoredSection("Partner Content")}
