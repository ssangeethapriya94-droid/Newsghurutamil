...
const SponsoredArticleDetail = () => {
  ...
  useEffect(() => {
    API.get(`/api/sponsored/article/${id}`)
      ...
      .catch((err) => {
        console.error(err);
        setError("ஸ்பான்சர் கட்டுரை கிடைக்கவில்லை அல்லது காலம் முடிவடைந்தது.");
      })
      .finally(() => setLoading(false));
  }, [id]);

  useSEO({
    title: article ? `${article.title} | நியூஸ் குரு ஸ்பான்சர்` : "ஸ்பான்சர் செய்தி | நியூஸ் குரு",
    description: article?.shortDescription || "நியூஸ் குருவில் ஸ்பான்சர் செய்யப்பட்ட செய்தியை வாசிக்கவும்.",
    keywords: "ஸ்பான்சர் செய்தி, நியூஸ் குரு விளம்பரம்",
  });

  if (loading) return <div style={{ padding: "60px", textAlign: "center" }}>ஸ்பான்சர் கட்டுரை ஏற்றப்படுகிறது...</div>;
  if (error || !article) return <div style={{ padding: "60px", textAlign: "center", color: "#ef4444" }}>{error || "செய்தி கிடைக்கவில்லை"}</div>;
  ...
  return (
    <article style={{ maxWidth: "1200px", margin: "0 auto", padding: "30px 20px", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ fontSize: "0.85rem", color: "#64748b", marginBottom: "16px", display: "flex", gap: "6px", alignItems: "center" }}>
        <Link to="/" style={{ color: "inherit", textDecoration: "none" }}>முகப்பு</Link>
        <span>&gt;</span>
        <span style={{ color: "inherit" }}>{article.category || "Sponsored"}</span>
        <span>&gt;</span>
        <span style={{ color: "#0f172a", fontWeight: "600" }}>விளம்பரக் கட்டுரை</span>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ fontSize: "0.9rem", color: "#64748b" }}>
          எழுதியவர்: <strong style={{ color: "#0f172a" }}>{article.reporterId?.name || "நியூஸ் குரு குழு"}</strong> &nbsp;•&nbsp; {new Date(article.publishedAt || article.createdAt).toLocaleDateString("ta-IN", { day: 'numeric', month: 'long', year: 'numeric' })}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ fontSize: "0.85rem", fontWeight: "700", color: "#64748b" }}>பகிர்</span>
          ...
          {article.hasVideo && article.videoUrl && (
            <div style={{ background: "#000", borderRadius: "12px", overflow: "hidden", marginBottom: "30px", boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
              <div style={{ background: "#1e293b", padding: "10px 16px", color: "#fdba74", fontWeight: "800", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "8px" }}>
                <FiPlay /> சிறப்பு விளம்பர வீடியோ (Promotional Video)
              </div>
          ...
          <div style={{ 
            ...
            <div style={{ textAlign: "center", marginBottom: "20px" }}>
              ...
              <p style={{ fontSize: "0.85rem", color: "#64748b", margin: 0, lineHeight: "1.5" }}>
                {article.eventDetails || "தரமான தயாரிப்புகள் மற்றும் தொழில்முறை சேவைகளை வழங்க உறுதிபூண்ட ஒரு முன்னணி கூட்டாளி."}
              </p>
            </div>
              ...
              <div style={{ borderTop: "1.5px solid #f1f5f9", paddingTop: "20px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  {article.socialLinks?.facebook && (
                    <a href={article.socialLinks.facebook} target="_blank" rel="noreferrer" title="Facebook-ல் பின்தொடரவும்"
                  ...
                  {article.socialLinks?.twitter && (
                    <a href={article.socialLinks.twitter} target="_blank" rel="noreferrer" title="Twitter / X-ல் பின்தொடரவும்"
                  ...
                  {article.socialLinks?.instagram && (
                    <a href={article.socialLinks.instagram} target="_blank" rel="noreferrer" title="Instagram-ல் பின்தொடரவும்"
                  ...
                  {article.socialLinks?.linkedin && (
                    <a href={article.socialLinks.linkedin} target="_blank" rel="noreferrer" title="LinkedIn-ல் பின்தொடரவும்"
                  ...
                  {article.socialLinks?.youtube && (
                    <a href={article.socialLinks.youtube} target="_blank" rel="noreferrer" title="YouTube-ல் சந்தாதாரராகவும்"
